...
export namespace JCS::XWUtils
{
	std::optional<std::wstring> DeletionStatusToWString(std::optional<XWF::Miscellaneous::XWF_Item_Info_Deletion_Status> deletionStatus)
	{
		if (!deletionStatus)
		{
			return std::nullopt;
	...
	std::optional<std::wstring> HashTypeToWString(std::optional<XWF::Miscellaneous::XW_HashType> hashType)
	{
		if (!hashType)
		{
			return std::nullopt;
	...
	/// <returns></returns>
	int HashTypeSize(XWF::Miscellaneous::XW_HashType hashType)
	{
		switch (hashType)
		{
			case XWF::Miscellaneous::XW_HashType::undefined:
	...
	std::optional<std::wstring> FileSystemTypeToWString(std::optional<XWF::Miscellaneous::XW_FileSystemType> fileSystemType)
	{
		if (!fileSystemType)
		{
			return std::nullopt;
	...
	std::wstring XTensionOperationTypeToWString(DWORD nOpType)
	{
		switch (nOpType)
		{
	...
	std::optional<std::wstring> GetUserInputText(const std::wstring& description = L"Please enter a value:", std::wstring defaultInput = std::wstring())
	{
		...
		std::optional<INT64> returnedStringSize = JCS::XWFWrapper::Miscellaneous::XWF_GetUserInput(
	...
	/// <returns>The positive integer value provided by the user. std::nullopt</returns>
	std::optional<INT64> GetUserInputNumber(const std::wstring& description = L"Please enter a value:")
	{
		std::optional<INT64> returnedNumber = JCS::XWFWrapper::Miscellaneous::XWF_GetUserInput(
			(LPWSTR)description.data(),
			NULL,
			NULL,
